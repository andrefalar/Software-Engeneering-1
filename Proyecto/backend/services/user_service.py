...
class UserService:
    ...
    def register_user(self, username: str, password: str) -> dict:
        ...
        try:
            ...
            # Crear nuevo usuario
            new_user = Usuario(
                username=username, password_hash=password_hash.decode("utf-8")
            )
            ...
            self._log_event(
                session, new_user.id_usuario, "Usuario registrado exitosamente"
            )
        ...
        try:
            ...
            if not user:
                self._handle_failed_attempt(
                    session, None, "Intento de login con usuario inexistente"
                )
                return {
                    "success": False,
            ...
            if bcrypt.checkpw(password_bytes, stored_hash):
                ...
                self._handle_failed_attempt(
                    session, user.id_usuario, "Intento de inicio de sesión fallido"
                )
                return {
                    "success": False,
    ...
    def _handle_failed_attempt(self, session: Session, user_id: int, description: str):
        ...
        if user_id:
            self._log_event(
                session,
                user_id,
                f"{description} (Intento {self.failed_attempts}/{self.max_failed_attempts})",
            )
        ...
        if self.failed_attempts >= self.max_failed_attempts:
            ...
            if user_id:
                self._log_event(
                    session, user_id, "Cuenta bloqueada por intentos fallidos"
                )
    ...
    def change_password(
        ...
        try:
            ...
            session.rollback()
            return {"success": False, "message": f"Error al cambiar contraseña: {e}"}
        finally:
            session.close()
        ...
        try:
            ...
            if not bcrypt.checkpw(password_bytes, stored_hash):
                self._log_event(
                    session,
                    user_id,
                    "Intento de eliminación de cuenta fallido - contraseña incorrecta",
                )
            ...
            archivos = (
                session.query(Archivo).filter(Archivo.usuario_id == user_id).all()
            )
            ...
            return {"success": False, "message": f"Error al eliminar cuenta: {e}"}
        finally:
            session.close()
    ...
    def get_security_status(self) -> dict:
        ...
        return {
            ...
            "remaining_attempts": max(
                0, self.max_failed_attempts - self.failed_attempts
            ),
        }
