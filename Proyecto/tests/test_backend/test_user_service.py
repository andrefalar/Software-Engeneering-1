...
class TestUserService:
    ...
    def test_user_registration_basic(self, user_service):
        ...
        assert (
            result["success"]
        ), f"Registro debería ser exitoso: {result['message']}"
        ...
        assert (
            user_exists
        ), "Debería existir al menos un usuario después del registro"
    ...
    def test_user_registration_restrictions(self, user_service):
        ...
        ), "No debería permitir registro de segundo usuario"
    ...
    def test_user_authentication_success(self, user_service):
        ...
        assert (
            auth_result["success"]
        ), f"Autenticación debería ser exitosa: {auth_result['message']}"
        assert "user_id" in auth_result, "Resultado debería incluir user_id"
        assert (
            auth_result["user_id"] == register_result["user_id"]
        ), "user_id debería coincidir"
    ...
    def test_user_authentication_failure(self, user_service):
        ...
        ), "Autenticación con contraseña incorrecta debería fallar"
    ...
    def test_user_info_retrieval(self, user_service):
        ...
        assert user_info["success"], "Obtención de info debería ser exitosa"
        ...
        print(
            f"   ✅ Info usuario: {user_info['username']} creado el {user_info['fecha_creacion']}"
        )
    ...
    def test_password_validation_strong(self, user_service):
        ...
        assert (
            result["success"]
        ), f"Contraseña fuerte debería ser aceptada: {result['message']}"
    ...
    def test_password_change(self, user_service):
        ...
        change_result = user_service.change_password(
            user_id, original_password, new_password
        )

        if hasattr(user_service, "change_password"):
            assert (
                change_result["success"]
            ), f"Cambio de contraseña debería ser exitoso: {change_result['message']}"
            ...
            assert (
                auth_old["success"] == False
            ), "Login con contraseña anterior debería fallar"
    ...
    def test_user_events_logging(self, user_service):
        ...
        if hasattr(user_service, "get_user_events"):
            ...
            ), "Obtención de eventos debería ser exitosa"
    ...
    def test_security_status(self, user_service):
        ...
        if hasattr(user_service, "get_security_status"):
            ...
            assert (
                "account_locked" in security_status
            ), "Estado debería incluir account_locked"
            assert isinstance(
                security_status["account_locked"], bool
            ), "account_locked debería ser bool"
    ...
    def test_password_validation_comprehensive(self, user_service):
        ...
        for password, should_be_valid, description in test_passwords:
            if hasattr(user_service, "_validate_password"):
                ...
                print(
                    f"   {status} '{password}' ({description}): {validation['message']}"
                )

                assert (
                    result
                ), f"Validación de '{password}' debería ser {should_be_valid}"
            else:
                ...
                if not should_be_valid:
                    ...
                    print(
                        f"   ✅ '{password}' ({description}): rechazada correctamente"
                    )
